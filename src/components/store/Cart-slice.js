...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addItemToCart(state, action) {
      ...
      state.changed = true;
      if (!existingItem) {
        // Ha nincs ilyen (nem része a tömbömnek)...
        state.items.push({
          // ...akkor az "items" tömbömbe felveszem egy új termékként, megadva neki az összes szükséges paramétert, ami rajtad múlik, hogy miket szeretnél hozzáadni.
          id: newItem.id,
          price: newItem.price,
          quantity: 1,
    ...
    removeItemFromCart(state, action) {
      ...
      if (existingItem.quantity === 1) {
        // Ha a létező termék mennyiség egyenlő egyel, akkor el kell távolítani a teljes elemet a tömmből.
